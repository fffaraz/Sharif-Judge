...
<div id="main_container">
	...
	<div id="page_title">
		...
		<?php if ($user_level>=2): ?>
		<span class="title_menu_item"><a href="<?php echo site_url('assignments/add') ?>"><i class="splashy-add_small"></i> <?php t("Add"); ?></a></span>
		<?php endif ?>
	</div>
		...
		<?php foreach($all_assignments as $item): ?>
			<div class="assignment_block" id="<?php echo $item['id'] ?>">
				...
				<div class="assignment_item">
					...
					<div class="assignment_subitem"><?php echo $item['problems'] ?> <?php t("problem"); ?><?php echo $item['problems']!=1?'s':'' ?></div>
					<div class="assignment_subitem"><?php echo $item['total_submits'] ?> <?php t("submit"); ?><?php echo $item['total_submits']!=1?'s':'' ?></div>
					<div class="assignment_subitem"><?php
						...
						if ($delay>$item['extra_time'])
							echo '<span style="color: red;"><?php t("Finished"); ?></span>';
						else
							echo $coefficient." %";?>
					</div>
					...
					<?php if($item['open']): ?>
						<span style="color: green;"><?php t("Open"); ?></span>
					<?php else: ?>
					...
					<?php endif ?>
					<?php if ($user_level>=1): ?>
						<div class="assignment_subitem_zero"><a href="<?php echo site_url('assignments/download/'.$item['id']) ?>"><i title="<?php t("Download Final Codes"); ?>" class="splashy-download"></i></a></div>
					<?php endif ?>
					<?php if ($user_level>=2): ?>
						<div class="assignment_subitem_zero"><a href="<?php echo site_url('moss/'.$item['id']) ?>"><i title="<?php t("Detect Similar Codes"); ?>" class="splashy-shield"></i></a></div>
					<?php endif ?>
					<?php if ($user_level>=2): ?>
						<div class="assignment_subitem_zero"><a href="<?php echo site_url('assignments/edit/'.$item['id']) ?>"><i title="<?php t("Edit"); ?>" class="splashy-pencil"></i></a></div>
					<?php endif ?>
					<?php if ($user_level>=2): ?>
						<div class="assignment_subitem_zero"><a href="<?php echo site_url('assignments/delete/'.$item['id']) ?>"><i title="<?php t("Delete"); ?>" class="splashy-remove_outline"></i></a></div>
					<?php endif ?>
				</div>
				</div>
		<?php endforeach ?>
